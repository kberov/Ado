...
A database is bundled in the distribution at C<etc/ado.sqlite>
to get started quickly. The used RDBMS is L<DBD::SQLite>.
...
Ado can be downloaded from CPAN L<http://search.cpan.org/dist/Ado/>
or installed via the C<cpan> commandline tool.
...
To use c<perldoc> for reading documentation you need to add 
the full path to L<Ado> C<lib/> directory to C<PERL5LIB> environment variable.
...
One way to contribute to L<Ado> is by writing plugins.
They work the same way as L<Mojolicious::Plugins> and share 
...
See L<Ado::Manual::Plugins> and L<Ado::Plugin> 
