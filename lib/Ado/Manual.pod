...
Ado is a framework for web projects based on L<Mojolicious>,
...
a model layer - L<Mojolicious::Plugin::DSC>. An SQLite database is bundled 
in the distribution at C<etc/ado.sqlite> to get started quickly. 
...
B<better> install it in its own folder.
...
and installed manually or installed directly from CPAN using C<cpan> or C<cpanm> commandline tools.
