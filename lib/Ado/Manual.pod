...
Ado is based on L<Mojolicious> and implements the recommendations from L<Mojolicious::Guides>. 
It adds some default configuration and a database schema. 
A default basic database is bundled in the distribution to get you 
up to speed quickly. The used RDBMS is L<DBD::SQLite>.
Tables can be accessed and manipulated using L<DBI> or any ORM.
We prefer L<DBIx::Simple::Class> trough
L<Mojolicious::Plugin::DSC>. It is not an ORM but provides representation of table records as plain Perl objects.
