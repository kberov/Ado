...
Ado it self consists of a simple core. It is distributed together with a few plugins to make it usable as
a basic Mojolicious application. Theoretically all of the core plugins could be switched off so you can
start only with a bare (I<but full>) L<Mojolicious> application if you wish. Later you can decide to
switch on some of them and eventually add your own L<Mojolicious> or L<Ado> plugins. Here is how it looks.
...
plugins can retrieve their settings from their own configuration files.

The configuration file must be named after the respective plugin.
...
depending on the current mode (C<$ENV{MOJO_MODE}>). The file 
